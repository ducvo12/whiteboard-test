...
import { CODEX_BASE_INSTRUCTIONS, CODEX_CONFIG_ARGS, CODEX_POLICY_VERSION } from "./codex-policy";
...
class CodexServer {
  ...
  async answer(prompt: string, signal: AbortSignal) {
    await this.ready;
...
export async function generateResponse(prompt: string, signal: AbortSignal) {
  ...
  try { return await shared.whiteboardCodex.answer(prompt, signal); }
  catch (error) {
    // A failed initialization must not poison the singleton for later requests.
    await shared.whiteboardCodex.ready.catch(() => shared.whiteboardCodex?.stop());
    throw error;
  }
}
